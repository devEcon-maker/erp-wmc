    ...
    <div class="flex justify-between items-center bg-surface-dark border border-[#3a2e24] p-6 rounded-2xl">
        <div>
            <h2 class="text-2xl font-bold text-white">
                {{ $order->exists ? 'Modifier la commande ' . $order->reference : 'Nouvelle Commande' }}
            </h2>
            <p class="text-text-secondary text-sm">Gestion des commandes clients.</p>
        </div>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <x-ui.card title="Informations Générales">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Date -->
                <x-ui.input type="date" label="Date de commande" wire:model="order_date" 
                    :error="$errors->first('order_date')" />
            </div>

             <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                <x-ui.input type="date" label="Date de livraison prévue" wire:model="delivery_date" 
                    :error="$errors->first('delivery_date')" />
                
                <x-ui.textarea label="Adresse de livraison" wire:model="shipping_address" rows="2" placeholder="Adresse complète..." />
             </div>
        ...
        <div class="flex justify-end gap-4 border-t border-[#3a2e24] pt-6">
            <x-ui.button type="secondary" href="{{ route('crm.orders.index') }}">Annuler</x-ui.button>
            <x-ui.button type="primary" :submit="true" wire:loading.attr="disabled">
                <span wire:loading.remove>Enregistrer la commande</span>

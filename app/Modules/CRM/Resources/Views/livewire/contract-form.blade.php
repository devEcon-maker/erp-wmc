    ...
    <div class="flex justify-between items-center bg-surface-dark border border-[#3a2e24] p-6 rounded-2xl">
        <div>
            <h2 class="text-2xl font-bold text-white">
                {{ $contract->exists ? 'Modifier le contrat ' . $contract->reference : 'Nouveau Contrat' }}
            </h2>
            <p class="text-text-secondary text-sm">Gestion des engagements.</p>
        </div>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <x-ui.card title="Informations Générales">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Type -->
                <x-ui.select label="Type de Contrat" wire:model.live="type">
                    <option value="contract">Contrat Fixe (Ponctuel)</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                 <!-- Dates -->
                <x-ui.input type="date" label="Date de début" wire:model="start_date" 
                    :error="$errors->first('start_date')" />
                
                <x-ui.input type="date" label="Date de fin (Optionnel)" wire:model="end_date" 
             ...
             <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                <x-ui.select label="Fréquence de facturation" wire:model="billing_frequency">
            ...
            <div class="mt-4">
                <x-ui.textarea label="Conditions Particulières" wire:model="terms" rows="2" placeholder="Termes spécifiques..." />
            </div>
                ...
                <x-ui.card>
                    <div class="space-y-4">
                        ...
                        <div class="border-t border-[#3a2e24] pt-4 mt-4">
                            ...
                            @if($type === 'subscription')
                                <p class="text-xs text-right text-text-secondary mt-1">facturé {{ match($billing_frequency) {'monthly' => 'mensuellement', 'quarterly' => 'trimestriellement', 'yearly' => 'annuellement', default => ''} }}</p>
                            @endif
                        </div>
                    </div>
                </x-ui.card>
        ...
        <div class="flex justify-end gap-4 border-t border-[#3a2e24] pt-6">
            <x-ui.button type="secondary" href="{{ route('crm.contracts.index') }}">Annuler</x-ui.button>
            <x-ui.button type="primary" :submit="true" wire:loading.attr="disabled">
                <span wire:loading.remove>Enregistrer le contrat</span>
